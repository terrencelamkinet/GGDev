...
def build_parser(state):
    ...
    def on_eeg(data):
        ...
        if getattr(data, 'attention', None) is not None and data.attention >= 0:
            ...
            if _eeg_count[0] <= 3 or _eeg_count[0] % 20 == 0:
                log_debug(f"STATE-updated: att={state.attention} sig={state.signal}")
        else:
            if _eeg_count[0] <= 3 or _eeg_count[0] % 20 == 0:
                log_debug(f"STATE-skip (no data)")
                ...
                if relay:
                    try:
                        ...
                        if int(time.time()) % 10 == 0:
                            log_debug(f"relay sent: att={state.attention} sig={state.signal}")
                    except Exception as e:
                        log_debug(f"relay send error: {e}")

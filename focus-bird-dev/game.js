...
/* ── Bird ─────────────────────────────────────────────────── */
const Bird = {
  x:0, y:0, vy:0, wingPhase:0, groundPhase:0, onGround:false,
  init(){ this.x=W*.22; this.y=H*.42; this.vy=0; this.groundPhase=0; this.onGround=false; },
  update(focusLvl, prof){
    this.wingPhase += .1;
    if(G.space || (G.extMode && focusLvl >= G.threshold)){
      /* FOCUSED → SINK toward ground food with strong pull */
      const pull = G.extMode
        ? prof.grav * (((focusLvl-G.threshold)/(100-G.threshold+prof.grace))*2.5+0.8)
        : prof.grav * 3.0;
      this.vy += pull;
    } else {
      /* NOT focused → FLOAT UP away from food with strong lift */
      this.vy -= prof.grav * 2.5;
    }
    this.vy = Math.max(-6, Math.min(8, this.vy));
    const prevY = this.y;
    this.y  = Math.max(32, Math.min(this.GROUND_Y(), this.y+this.vy));
    /* Track whether bird is resting on ground */
    this.onGround = (this.y >= this.GROUND_Y() - 1 && this.vy > 0);
    if(this.onGround){ this.vy = 0; this.groundPhase += .06; }
    else { this.groundPhase = 0; }
  },
  GROUND_Y(){ return H - 54 - FOOD_SIZE; },
  draw(nearFoodY){
    const cx=this.x, cy=this.y + (this.onGround ? Math.sin(this.groundPhase)*3 : 0);
